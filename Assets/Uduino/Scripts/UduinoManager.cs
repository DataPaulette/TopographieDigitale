...
#if UDUINO_READY
            Instance = this;
            UduinoInterface.Instance.Create();
        ...
        */
        ...
        public void CloseDevice(UduinoDevice device)
        {
            try
            {
                ...
                if (Application.isPlaying)
                {
                    UduinoInterface.Instance.RemoveDeviceButton(device.name);
                    UduinoInterface.Instance.UduinoDisconnected(device.name);
                }
                ...
                if (Application.isPlaying && uduinoDevices.Count == 0)
                    StopThread();
